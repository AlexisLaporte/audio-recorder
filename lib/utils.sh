...
show_help() {
    ...
    echo "  stop --trim            Stop, auto-trim silence, then process"
    ...
    echo "  trim [folder]          Detect conversation end & trim trailing silence"
    echo "  transcribe [path]      Transcribe audio (folder or file)"
    echo "  summarize [folder]     Generate AI summary (uses CLAUDE.md if in project dir)"
